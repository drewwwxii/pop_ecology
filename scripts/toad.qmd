...
ggplot() +
  ...
  geom_sf(data = burn_scars, fill = "red", color = "black", alpha = 0.5) +
  geom_sf(data = hr95, aes(geometry = geometry), fill = "blue", color = "blue", alpha = 0.3) +  # KDE 95% Home Range
  geom_sf(data = hr50, aes(geometry = geometry), fill = "purple", color = "purple", alpha = 0.5) +  # KDE 50% Core Area
  geom_sf(data = mcp95, aes(geometry = geometry), fill = "yellow", color = "orange", alpha = 0.3) +  # MCP Home Range
  geom_sf(data = cleveland_toad_sf, aes(geometry = geometry), color = "black", size = 1, alpha = 0.7) +  # Observation points
  labs(title = "Cleveland National Forest - Toad Home Range & Fire Scars",
       subtitle = "Blue = 95% KDE | Purple = 50% KDE | Yellow = MCP | Red = Fire Scars",
       x = "Longitude", y = "Latitude") +
  theme_minimal()
```

# Easier to understand mapping 
```{r}
# 1️⃣ KDE Home Range over Cleveland National Forest
p1 <- ggplot() +
  geom_sf(data = clevelandNF, fill = "lightgreen", color = "darkgreen", alpha = 0.5) +
...
# 2️⃣ Burn Scars over Cleveland National Forest
p2 <- ggplot() +
  geom_sf(data = clevelandNF, fill = "lightgreen", color = "darkgreen", alpha = 0.5) +
